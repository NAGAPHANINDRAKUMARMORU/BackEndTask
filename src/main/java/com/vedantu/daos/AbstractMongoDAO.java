...
public abstract class AbstractMongoDAO {
    ...
    public <T extends AbstractMongoEntity> T getEntityByemail(String email, Class<T> clazz) {
        Query query = new Query(Criteria.where("email").is(email));
        return getMongoOperations().findOne(query, clazz, clazz.getSimpleName());
    }
    
    public <T extends AbstractMongoEntity> T getEntityByname(String name, Class<T> clazz) {
        Query query = new Query(Criteria.where("productName").is(name));
        return getMongoOperations().findOne(query, clazz, clazz.getSimpleName());
    }
   
    
    public <T extends AbstractMongoEntity> T getEntityByCId(String cid, Class<T> clazz) {
        Query query = new Query(Criteria.where("cartCustomerId").is(cid));
        return getMongoOperations().findOne(query, clazz, clazz.getSimpleName());
    }
    ...
    public <T extends AbstractMongoEntity> List<T> getAllCustomers(Class<T> clazz) {
        return getMongoOperations().findAll(clazz,clazz.getSimpleName());
    }
    
    public <T extends AbstractMongoEntity> Collection<T> getAllDataFromCorrespondingCollection(Class<T> clazz) {
        return getMongoOperations().findAll(clazz,clazz.getSimpleName());
    }
